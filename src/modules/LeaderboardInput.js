export class LeaderboardInput {
    ...
    leaderboardEntry(width, height, scaleX, scaleY, level) {
        ...
        // Arrow Buttons
        this.p5.image(this.upButton, width * 0.77, height * 0.6, 60 * scaleX, 60 * scaleY);
        this.p5.image(this.downButton, width * 0.77, height * 0.8, 60 * scaleX, 60 * scaleY);
        this.p5.image(this.leftButton, width * 0.71, height * 0.7, 60 * scaleX, 60 * scaleY);
    ...
    leaderboardState(x, y, width, height, scaleX, scaleY) {
        ...
        // Up Button
        else if ((width * 0.77) < x && x < (width * 0.77 + 60 * scaleX) && y > (height * 0.6) && y < (height * 0.6 + 60 * scaleY)) {
            this.soundManager.playLeaderboardClick();
            if (this.letterSelector < 13) {
                // Goes down one row to the next character
                this.letterSelector += 13;
        ...
        // Right Button
        else if ((width * 0.83) < x && x < (width * 0.83 + 60 * scaleX) && y > (height * 0.7) && y < (height * 0.7 + 60 * scaleY)) {
            this.soundManager.playLeaderboardClick();
            if (this.letterSelector == 12 || this.letterSelector == 25) {
                // Loops the row at the end
                this.letterSelector -= 12;
        ...
        // Down Button
        else if ((width * 0.77) < x && x < (width * 0.77 + 60 * scaleX) && y > (height * 0.8) && y < (height * 0.8 + 60 * scaleY)) {
            this.soundManager.playLeaderboardClick();
            if (this.letterSelector < 13) {
                // Goes down one row to the next character
                this.letterSelector += 13;
